...
class AdminPageHandler
{
	...
	public function process(Page $page)
	{
		if ($this->request->getMethod() == 'POST')
		{
			...
			if ($this->form->isValid())
			{
				...
				$page->setSlug($this->makeSlug($page));
	 ...
	 * @return string
	 */
	private function addSuffixNumber($slug, Page $page, $number = 0)
	{
		$em = $this->doctrine->getEntityManager();
		$slugTmp = $number > 0 ? $slug . self::SLUG_SUFFIX_SEPARATOR . $number : $slug;
		$parentFullpath = $page->getParent()->getFullpath();
		$foundedPage = $this->doctrine->getRepository('FulgurioLightCMSBundle:Page')->findOneByFullpath(($parentFullpath != '' ? ($parentFullpath . '/') : '') . $slugTmp);
	 ...
	 * @return string
	 */
	final public function makeSlug($page)
	{
		$title = $page->getTitle();
